...
void declareIdList(AST_NODE* declarationNode, SymbolAttributeKind isVariableOrTypeAttribute, int ignoreArrayFirstDimSize)
{
    ...
    while(traverseIDList)
    {
        ...
        else
        {
            ...
            attribute->offset = -1;//added by zhuqi
            ...
            else
            {
                ...
                if(isVariableOrTypeAttribute == VARIABLE_ATTRIBUTE && !ignoreArrayFirstDimSize && !isGlobalVar(traverseIDList->semantic_value.identifierSemanticValue.symbolTableEntry))
                {
                    set_offset(attribute);
                }
            }
        }
...
void processBlockNode(AST_NODE* blockNode)
{
    int offsetBeforeProcessNewBlock = offset;
    openScope();
    ...
    offset = offsetBeforeProcessNewBlock;
}
...
void declareFunction(AST_NODE* declarationNode)
{
    ...
    if(!errorOccur)
    {
        ...
        attribute->offset =  (largest < offset) ? largest : offset;//added by zhuqi
    }
