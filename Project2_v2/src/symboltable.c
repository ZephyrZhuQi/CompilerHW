...
int compare_id(const void* left, const void* right)
{
	return strcmp((*(const symtab**)left)->lexeme,(*(const symtab**)right)->lexeme);
}

void printSymTab()
{
	symtab **nodelist = malloc(gVocabCount*sizeof(symtab*));
    int i;
	int idx=0;
    //printf("----- Symbol Table ---------\n");
	...
	//qsort()
	qsort(nodelist,idx,sizeof(symtab*),compare_id);
